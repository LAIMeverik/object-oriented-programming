...
class Dot:
    ...
    def __init__(self):             # Инициализация
        ...
        if n > 1:
            ...
            r = float(input('Введите расстояние от начала координат точки: '))
            o = float(input('Введите зенитный угол точки: '))
            fi = float(input('Введите азимутный угол точки: '))
            self.cord = [r, o, fi]
...
n = int(input('В каком пространстве вы хотите работать? (n -- натуральное число)'
              ...
              '\nn < 1 -- сферическая система координат'
              '\nn = '))
...
sorted_f = list()   # Лист для сортировки
areas = list()      # Лист для значений площадей
for i in figures:   # Заполнение листа для площадей и его сортировка
    areas.append(i.area())
areas.sort()

for s in areas:     # Не супер эффективный алгоритм сортировки, не трогайте его, он старается
    for j in figures:
        if j.area() == s:
            sorted_f.append(j)
            figures.remove(j)
